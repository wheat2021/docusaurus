...
- `path`: 相对于插件本地化文件夹 `i18n/[locale]/[pluginName]`。扩展名 `.json` 应省略以保持通用性。
- `content`: 使用 Chrome i18n JSON 格式。

这些文件将由 [`write-translations` CLI](../../cli.mdx#docusaurus-write-translations-sitedir) 写入插件的 i18n 子文件夹，并在调用 [`translateContent()`](#translateContent) 和 [`translateThemeConfig()`](#translateThemeConfig) 之前以适当的语言区域读取
...
它应以 `Record<string, string>` 的形式返回消息，其中键是翻译 ID，值是使用网站当前语言区域本地化的消息（不含描述）。
